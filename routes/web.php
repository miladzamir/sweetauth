...
Route::group(['namespace' => 'MiladZamir\SweetAuth\Http\Controllers\Handler', 'middleware' => 'web'], function () {
    ...
    Route::get(config('swauth.viewRouteNames.step3.0'), 'ViewHandlerController@stepThreeMethodZero')->name(config('swauth.viewRouteNames.step3.0'))->middleware('is.step.three.method.zero');
    Route::get(config('swauth.viewRouteNames.step3.1'), 'ViewHandlerController@stepThreeMethodOne')->name(config('swauth.viewRouteNames.step3.1'))->middleware('is.step.three.method.one');
...
Route::group(['namespace' => 'MiladZamir\SweetAuth\Http\Controllers', 'middleware' => 'web'], function () {
    ...
    Route::post(config('swauth.postRouteNames.step3.0'), 'StepThreeController@configure')->name(config('swauth.postRouteNames.step3.0'))->middleware('is.step.three.method.post');
});

Route::group(['namespace' => 'App\Http\Controllers\Auth', 'middleware' => 'web'], function () {
    Route::post(config('swauth.postRouteNames.step4.0'), 'LoginController@login')->name(config('swauth.postRouteNames.step4.0'));
